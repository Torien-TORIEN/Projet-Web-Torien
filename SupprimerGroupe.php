<?php
   ...
   if($_SESSION["suppG"]=="not ok"){
        ...
        <strong>ERROR 404 !</strong> Ce groupe n\'existe pas!
      </div>';}
    if ($_SESSION["suppG"]=="ok"){
    $erreur='<div class="alert alert-success alert-dismissible">
    <button type="button" class="close" data-dismiss="alert">&times;</button>
    <strong>Notification :</strong> Un parmi vos groupes a été supprimé avec succes !
  </div>';}
...
<main role="main">
        <div class="jumbotron" style="background-image:url('img/im4.jpg')">
            <div class="container" style="color:green;text-align:center;border:10px solid red; border-radius:30px">
              <h1 class="display-4" style="border-style:none solid none solid;border-color:drey;border-radius:20px;">Supprimer un Groupe</h1>
              <p style="color:black">Taper le  Groupe à supprimer!</p>
            </div>
          </div>
    ...
    <div class="erreur"> <?php echo $erreur;?></div>
            <form action="SupprimerG.php" method="GET"  >
                ...
                <select  id="classe" name="classe"  class="custom-select custom-select-sm custom-select-lg">
                   ...
                   <button  type="submit" class="btn btn-primary btn-block" name="supprimer" onclick="return confirm('Do you want to delete this Group?')">supprimer</button>
                </div>
            </form>
        <!--Bouton Supprimer-->
